...
#Plot results
plt.scatter(x, f,color='blue',label = "f(x)")
plt.scatter(x,y,color='red',label = "Noisy measurements y")
plt.scatter(x,f_hat,color='green',label = "Estimated f(x)")
...
axs[2].scatter(x,f_hat,color='green',label = "Estimated f(x)")
axs[2].set_xlim(-1,+1)
...
plot6.plot(Tl_3, np.real(np.log(err11_3[0:T3-1])),color='purple')
plot6.plot(Tl_4,np.real(np.log(err11_4[0:T4-1])),color='blue')
# naming the x axis 
plt.xlabel('time')
